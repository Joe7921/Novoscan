...
/** 评分修正封顶值（与轮次联动，避免 MAX_ROUNDS 调整后逻辑失配） */
...
export function shouldTriggerDebate(agents: {
    ...
    // 筛选出分歧超过阈值的辩论对
    const triggeredPairs = potentialPairs.filter(p => p.divergence > DIVERGENCE_THRESHOLD);
    ...
    if (triggeredPairs.length === 0) {
        ...
        if (bestPair && maxDivergence > DIVERGENCE_THRESHOLD) {
            return {
                trigger: true,
        ...
        return {
            trigger: false,
            reason: `虽然整体标准差为 ${Math.round(stdDev)}，但所有 Agent 对的评分差异均 ≤ ${DIVERGENCE_THRESHOLD} 分，无需辩论`,
            pairs: []
        };
    }
...
export async function runDebateSession(
    ...
    for (let round = 1; round <= MAX_ROUNDS; round++) {
        ...
        if (onProgress) {
            onProgress('log', `[NovoDebate] ⚔️ ${sessionId} 第 ${round}/${MAX_ROUNDS} 轮开始（${currentChallenger.agentName} → ${currentDefender.agentName}）`);
        }
    ...
    // 计算评分修正（基于辩论结果）
    const scoreAdjustment = calculateScoreAdjustment(exchanges, proAgentOutput.agentName, conAgentOutput.agentName);
...
export function calculateScoreAdjustment(
    ...
    // 封顶值与轮次联动：cap = MAX_ROUNDS × ADJUST_PER_ROUND
    return {
        proAgentDelta: Math.max(-SCORE_ADJUSTMENT_CAP, Math.min(SCORE_ADJUSTMENT_CAP, proDelta)),
        conAgentDelta: Math.max(-SCORE_ADJUSTMENT_CAP, Math.min(SCORE_ADJUSTMENT_CAP, conDelta))
    };
}
...
export async function executeNovoDebate(
    ...
    const sessionPromises = pairs.map(pair => {
        if (onProgress) {

...
import { academicReviewer } from '@/agents/academicReviewer';
import { industryAnalyst } from '@/agents/industryAnalyst';
import { competitorDetective } from '@/agents/competitorDetective';
import { innovationEvaluator } from '@/agents/innovationEvaluator';
import { arbitrator } from '@/agents/arbitrator';
import { qualityGuard } from '@/agents/qualityGuard';
import { crossDomainScout, createFallbackCrossDomainOutput } from '@/agents/crossDomainScout';
import { executeNovoDebate, createFallbackDebateRecord } from '@/agents/debater';
...
async function handleFullRetry(
    ...
    if (!currentUserId) {
        return NextResponse.json(
            { success: false, error: '完全重试仅限已登录用户使用', requireLogin: true },
            { status: 401 }
        );
    }
...
async function handlePartialRetry(
    ...
    if (!currentUserId) {
        return NextResponse.json(
            { success: false, error: '动态计费重试仅限已登录用户使用', requireLogin: true },
            { status: 401 }
        );
    }

...
export default function ProfilePage() {
        ...
        const fetchData = async () => {
            try {
                ...
                if (json.success) {
                    ...
                    // 填充编辑字段
                    if (json.profile) {
                        setEditName(json.profile.displayName || user.user_metadata?.full_name || '');
    ...
    useEffect(() => {
        ...
        const fetchWallet = async () => {
            try {
                ...
                const json = await res.json();
                if (json.success) setWalletData(json);
            } catch (e) {
                console.error('获取钱包数据失败', e);
            }
        };
        fetchWallet();
    }, [authReady, user]);
    ...
    const displayName = editName || user?.user_metadata?.full_name || user?.email?.split('@')[0] || '用户';
            ...
            >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <motion.div variants={cardVariants} className="lg:col-span-1 space-y-6">
                        ...
                        {/* 钱包卡片 */}
                        <div className="bg-white/95 rounded-2xl p-6 border border-gray-100 shadow-sm overflow-hidden relative">
                            <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-bl from-amber-400/10 via-yellow-300/5 to-transparent rounded-bl-full" />
                            <h3 className="text-sm font-bold text-gray-500 uppercase tracking-wider mb-4 flex items-center gap-2 relative z-10">
                                <Wallet className="w-4 h-4 text-amber-500" />
                                NovoCredits 积分账户
                            </h3>
                            <div className="relative z-10">
                                <div className="flex items-baseline gap-2 mb-1">
                                    <span className="text-4xl font-black text-gray-900">{walletData?.points ?? prefsData?.profile?.points ?? 0}</span>
                                    <span className="text-sm font-bold text-amber-600">Credits</span>
                                </div>
                                <p className="text-xs text-gray-400 mb-4">可用于兑换分析次数</p>

                                {/* 最近流水 */}
                                {walletData?.recentTransactions && walletData.recentTransactions.length > 0 ? (
                                    <div className="space-y-2 border-t border-gray-100 pt-3">
                                        <p className="text-xs font-semibold text-gray-400 mb-2">最近记录</p>
                                    ...
                                    <p className="text-xs text-gray-300 border-t border-gray-100 pt-3">暂无交易记录</p>
                                )}
                                ...
                                <div className="border-t border-gray-100 pt-4 mt-3">
                                    ...
                                    <div className="flex gap-2">
                                        <input
                                            ...
                                            placeholder="输入兑换码"
                                            className="flex-1 px-3 py-2 text-xs border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-400/50 focus:border-amber-400 bg-white/95 font-mono tracking-wider"
                                            maxLength={20}
                                        ...
                                        <button
                                            onClick={async () => {
                                                ...
                                                try {
                                                    ...
                                                    const json = await res.json();
                                                    setRedeemResult({ success: json.success, message: json.message || json.error });
                                                    if (json.success) {
                                                        setRedeemCode('');
                                                        // 刷新钱包数据
                                                        setWalletData(prev => prev ? { ...prev, points: json.balance } : prev);
                                                        // 重新获取完整钱包数据
                                                        fetch('/api/wallet').then(r => r.json()).then(d => { if (d.success) setWalletData(d); });
                                                    }
                                                } catch {
                                            ...
                                            className="px-4 py-2 text-xs font-bold text-white bg-gradient-to-r from-amber-500 to-orange-500 rounded-lg hover:from-amber-600 hover:to-orange-600 transition-all disabled:opacity-40 disabled:cursor-not-allowed shadow-sm"
                                        >
                                    ...
                                    <AnimatePresence>
                                        {redeemResult && (
                                            <motion.p
                                                ...
                                                exit={{ opacity: 0 }}
                                                className={`text-xs mt-2 flex items-center gap-1 ${redeemResult.success ? 'text-emerald-600' : 'text-red-500'}`}
                                            >
                                                {redeemResult.success ? <CheckCircle className="w-3 h-3" /> : <AlertCircle className="w-3 h-3" />}
                                ...
                                <Link
                                    href="/novocredit"
                                    className="flex items-center justify-center gap-2 w-full mt-4 py-2.5 bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white rounded-xl font-bold text-xs transition-all duration-300 shadow-sm hover:shadow-md"
                                >
